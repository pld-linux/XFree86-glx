...
To jest implementacja biblioteki Mesa 3D oraz GLX z obsługą sprzętowej
akceleracji. GLX został stworzony przez SGI jako rozszerzenie systemu
X Window w celu integracji funkcji renderujących OpenGL. Pozwala to
bibliotece Mesa 3D na wykonywanie funkcji renderujacych 3D wewnątrz
procesu X serwera zamiast po stronie X klienta. Daje to potencjalne
zyski wydajności, ponieważ wyrenderowany obraz nie musi być przesyłany
z programu X klienta do X serwera (przesyłane są tylko polecenia
potrzebne do renderowania). Daje to także możliwość użycia sprzętowej
akceleracji 3D. Ten pakiet zawiera sterowniki wykorzystujące
akcelerację dla kart graficznych opartych na układach z serii NVIDIA
Riva i ATI Rage Pro.

Biblioteka graficzna Mesa 3D jest potężnym i ogólnym zestawem narzędzi
do tworzenia grafiki komputerowej przy wsparciu sprzętu. Mesa używa
składni poleceń i maszyny stanów OpenGL za zgodą Silicon Graphics,
Inc. Nie jest to jednak licencjonowana implementacja OpenGL.

Ten pakiet bazuje na Mesie %{mesaver} oraz utah-glx-%{glxver}.

Więcej informacji na stronie http://utah-glx.sourceforge.net/.
...
Mesa jest biblioteką 3D zgodną z OpenGL 1.2. Ten pakiet zawiera pliki
nagłówkowe potrzebne do kompilowania programów używających Mesy.
